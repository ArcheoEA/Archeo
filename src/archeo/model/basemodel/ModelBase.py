...
# The ModelBase class represents a model with attributes such as name, type, referenceId, sourcesId, and description.
...
# The hash of a ModelBase is based on its referenceId.
@dataclass ()
class ModelBase:
    name: str
    type: str
    ...
    # The __slots__ declaration is used to optimize memory usage by preventing the creation of a __dict__ for each instance of ModelBase.
    __slots__ = ['name', 'type', 'referenceId', 'sourcesId', 'description']

    # The constructor of ModelBase initializes the name, type, referenceId, sourcesId, and description attributes with validation to ensure that name, type, and referenceId are not empty or None.
    def __init__(self,
                pName: str,
                pType: str,
    ...
    def __post_init__(self):
        try:
            ...
            if (self.name is None) or (self.name == "") or (self.type is None) or (self.type == "") or (self.referenceId is None) or (not isinstance(self.referenceId, IdentityPart)):
                print(f"Post-initialization of ModelBase failed, with name={self.name}, type={self.type}, referenceId={self.referenceId}")
    ...
    def __hash__(self) -> int:
        try:
            ...
            if (self.name is None) or (self.name == "") or (self.type is None) or (self.type == "") or (self.referenceId is None) or (not isinstance(self.referenceId, IdentityPart)):
                print(f"Hash calculation of ModelBase failed, with name={self.name}, type={self.type}, referenceId={self.referenceId}")

                raise ValueError("Invalid name, type, or referenceId for hashing")
            
            # The hash is based on the referenceId of the ModelBase
    ...
    def __eq__(self, other: Any) -> bool:
        try:
            ...
            # Two ModelBase instances are considered equal if their referenceId attributes are equal
    ...
    def __repr__(self) -> str:
        try:
            ...
            if (self.name is None) or (self.name == "") or (self.type is None) or (self.type == "") or (self.referenceId is None) or (not isinstance(self.referenceId, IdentityPart)):
                print(f"String representation of ModelBase failed, with name={self.name}, type={self.type}, referenceId={self.referenceId}")

                raise ValueError("Invalid name, type, or referenceId for string representation")

            # The string representation of the ModelBase includes the name, type, referenceId, number of sourcesId, and description
            return f"ModelBase(name={self.name}, type={self.type}, referenceId={self.referenceId}, len(sourcesId)={len(self.sourcesId) if self.sourcesId else 0}, description={self.description})"
